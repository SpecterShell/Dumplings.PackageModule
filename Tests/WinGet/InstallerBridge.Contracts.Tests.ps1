...
'@
  ...
  It 'Should preserve Advanced Installer external-resource evidence through the parser bridge' {
    $Fixture = Resolve-DumplingsTestFixturePath -RelativePath 'Builders\AdvancedInstaller\6.3\Generated\diehard-external-63.exe'
  ...
  It 'Should preserve Advanced Installer MSI/MSIX platform-selection evidence through the parser bridge' {
    $Fixture = Resolve-DumplingsTestFixturePath -RelativePath 'Builders\AdvancedInstaller\23.9\Generated\mixed-controlled-Msi.exe'
    if (-not (Test-Path -LiteralPath $Fixture -PathType Leaf)) {
  ...
  It 'Should project Advanced Installer prerequisite execution and detection evidence' {
    $Fixture = Resolve-DumplingsTestFixturePath -RelativePath 'Builders\AdvancedInstaller\8.6\Generated\prereq-lzma-86.exe'
    if (-not (Test-Path -LiteralPath $Fixture -PathType Leaf)) {
  ...
  It 'Should distinguish force-install and downloadable Advanced Installer prerequisites' {
    $ForceFixture = Resolve-DumplingsTestFixturePath -RelativePath 'Builders\AdvancedInstaller\8.6\Generated\prereq-force-86.exe'
    $UrlFixture = Resolve-DumplingsTestFixturePath -RelativePath 'Builders\AdvancedInstaller\8.6\Generated\prereq-url-86.exe'
  ...
  It 'Should read a fixed-path Advanced Installer ARM64 payload through the bridge' {
    $Fixture = Get-DumplingsTestFixture -RelativePath (Resolve-DumplingsTestFixtureCatalogPath -Name 'fxsound_setup.arm64-1.2.10.0.exe') -Uri 'https://raw.githubusercontent.com/fxsound2/fxsound-app/refs/tags/v1.2.10.0/release/arm64/fxsound_setup.arm64.exe'
    $Info = Get-AdvancedInstallerInfo -Path $Fixture
