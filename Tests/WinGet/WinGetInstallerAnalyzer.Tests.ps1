...
BeforeAll {
  ...
  function Get-DeclaredTypeRegressionFixture {
    ...
    Get-DumplingsTestFixture -RelativePath (Resolve-DumplingsTestFixtureCatalogPath -Name $Name) -Uri $Url -Sha256 $Sha256
  }
...
Describe 'InstallShield structural analyzer routing' {
  It 'invokes the parser for an exact InstallShield 3 engine identity' {
    $Fixture = Resolve-DumplingsTestFixturePath -RelativePath 'Builders\InstallShield\3\ClassicMedia\setup32.exe'
...
Describe 'WinGet installer analyzer content detection' {
  ...
  It 'Should project markerless install4j application media through the WinGet analyzer' {
    $Installer = Resolve-DumplingsTestFixturePath -RelativePath 'Builders\Install4j\11\Generated\generated-install4j-11-nojre.exe'
