...
BeforeDiscovery {
  if (-not ([System.Management.Automation.PSTypeName]'Dumplings.Versioning.WinGetVersion').Type) {
    Add-Type -Path (Join-Path $Script:DumplingsModuleRoot 'Assets\Source\Versioning\Versioning.cs')
  }
  ...
  Import-Module (Join-Path $Script:DumplingsModuleRoot 'Libraries\WinGet\WinGetManifestValidation.psm1') -Force
}
...
'@ | Set-Content -LiteralPath $Path -Encoding utf8NoBOM
  ...
  It 'Loads every vendored schema offline' {
    $SchemaRoot = Join-Path $Script:DumplingsModuleRoot 'Assets\Schemas\WinGetManifest'
    ...
    Get-Content (Join-Path $Script:DumplingsModuleRoot 'Libraries\WinGet\WinGetManifestValidation.psm1') -Raw | Should -Not -Match 'winget\.exe\s+validate'
  }
}
...
Describe 'Process-safe validation' {
  It 'Validates concurrently in thread-job runspaces' {
    ...
    Write-TestSingletonManifest -Path $Path
    $ModuleRoot = $Script:DumplingsModuleRoot
