...
BeforeAll {
  ...
  if (-not ([System.Management.Automation.PSTypeName]'Dumplings.WebDriver.WebDriverLeasePool').Type) {
    Add-Type -Path (Join-Path $Script:DumplingsModuleRoot 'Assets' 'Source' 'WebDriver' 'WebDriverLeasePool.cs')
  }
...
Describe 'WebDriver task integration contracts' {
  It 'keeps WebDriver lifecycle integration in PackageModule hooks' {
    $Runner = Get-Content -LiteralPath (Join-Path $Script:DumplingsRepositoryRoot 'Core' 'Index.ps1') -Raw
    $HookRoot = Join-Path $Script:DumplingsModuleRoot 'Hooks'
...
Describe 'WebDriver lifecycle hooks' {
  BeforeAll {
    ...
    $Script:DumplingsRepositoryRoot = [IO.Path]::GetFullPath((Join-Path $Script:DumplingsModulesRoot '..'))
    . (Join-Path $Script:DumplingsTestRoot 'Support\TestFixture.ps1')
    $Script:HookWebDriverModule = Import-Module (Join-Path $Script:DumplingsModuleRoot 'Libraries' 'Browser' 'WebDriver.psm1') -Force -DisableNameChecking -PassThru
    $Script:HookRoot = Join-Path $Script:DumplingsModuleRoot 'Hooks'
  }
...
Describe 'Scoped WebDriver PowerShell API' {
  BeforeAll {
    ...
    $Script:DumplingsRepositoryRoot = [IO.Path]::GetFullPath((Join-Path $Script:DumplingsModulesRoot '..'))
    . (Join-Path $Script:DumplingsTestRoot 'Support\TestFixture.ps1')
    $Script:WebDriverModule = Import-Module (Join-Path $Script:DumplingsModuleRoot 'Libraries' 'Browser' 'WebDriver.psm1') -Force -DisableNameChecking -PassThru
  }
...
Describe 'Shared WebDriver real runspace smoke' -Tag Integration {
  It 'uses one real Selenium session and service across thread-job runspaces' -Skip:($env:DUMPLINGS_WEBDRIVER_INTEGRATION -ne '1') {
    $ModulePath = (Resolve-Path (Join-Path $Script:DumplingsModuleRoot 'Libraries' 'Browser' 'WebDriver.psm1')).Path
