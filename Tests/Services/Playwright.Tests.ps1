...
BeforeAll {
  ...
  $Script:ModulePath = Join-Path $Script:DumplingsModuleRoot 'Libraries' 'Browser' 'Playwright.psm1'
...
Describe 'Scoped Playwright PowerShell API' {
  ...
  It 'does not expose a PowerShell route or event callback path' {
    ...
    $SessionSource = Get-Content -LiteralPath (Join-Path $Script:DumplingsModuleRoot 'Assets' 'Source' 'Playwright' 'PlaywrightSession.cs') -Raw
...
Describe 'Playwright runner lifecycle hooks' {
  ...
  It 'assigns and clears the same task owner without creating an unused pool' {
    & (Join-Path $Script:DumplingsModuleRoot 'Hooks' 'BeforeTask.ps1') -Context $Script:Context
    $Global:DumplingsPlaywrightLeaseOwnerId | Should -BeExactly 'DumplingsWok0/Test.Package/invocation'

    & (Join-Path $Script:DumplingsModuleRoot 'Hooks' 'AfterTask.ps1') -Context $Script:Context
  ...
  It 'disposes a Playwright pool during final runner shutdown' {
    ...
    & (Join-Path $Script:DumplingsModuleRoot 'Hooks' 'RunnerStopping.ps1') -Context $Script:Context
