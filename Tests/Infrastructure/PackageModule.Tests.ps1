...
Describe 'PackageModule manifest-backed loading' {
  BeforeAll {
    ...
    $Script:ManifestPath = [IO.Path]::GetFullPath((Join-Path $Script:DumplingsModuleRoot 'PackageModule.psd1'))
  ...
  It 'resolves help for commands loaded by the root module' {
    $CommandNames = Get-Module | Where-Object Path -Like "$Script:DumplingsModuleRoot\Libraries\*.psm1" | ForEach-Object { $_.ExportedFunctions.Keys } | Sort-Object -Unique
