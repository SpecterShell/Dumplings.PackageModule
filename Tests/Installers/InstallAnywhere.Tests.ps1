...
BeforeAll {
  ...
  Import-Module (Join-Path $Script:DumplingsModuleRoot 'Libraries' 'Infrastructure' 'InstallerEvidence.psm1') -Force
  Import-Module (Join-Path $Script:DumplingsModuleRoot 'Libraries' 'Installers' 'InstallAnywhere.psm1') -Force
...
'@
  ...
  It 'Should parse the source-backed built-in ARP key and registry actions from FlowJo 10' {
    $Fixture = Resolve-DumplingsTestFixturePath -RelativePath (Resolve-DumplingsTestFixtureCatalogPath -Name 'FlowJo-Win64-10.10.0.exe')
