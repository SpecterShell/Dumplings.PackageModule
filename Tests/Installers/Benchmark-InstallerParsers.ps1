...
$RepositoryRoot = [IO.Path]::GetFullPath((Join-Path $Script:DumplingsRepositoryRoot '.'))
...
$Results = @(
  if ($NSISPath) {
    if ($NSISBaselineModulePath -and (Test-Path -LiteralPath $NSISBaselineModulePath)) {
      ...
      Invoke-InstallerParserBenchmark -Name NSISBaseline -Path $NSISPath -Expression "if (-not `$env:windir) { `$env:windir = 'C:\Windows' }; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Runtime.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Binary.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\FileSystem.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Archive.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\PE.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\InstallerEvidence.psm1 -Force; Import-Module '$BaselineModule' -Force; Get-NSISInfo -Path `$InstallerPath"
    }
    Invoke-InstallerParserBenchmark -Name NSIS -Path $NSISPath -Expression "Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Runtime.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Binary.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\FileSystem.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Archive.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\PE.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\InstallerEvidence.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Installers\NSIS.psm1 -Force; Get-NSISInfo -Path `$InstallerPath"
  }
  if ($ChromiumPath) {
  ...
  if ($InnoPath) {
    Invoke-InstallerParserBenchmark -Name Inno -Path $InnoPath -Expression "Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Runtime.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Binary.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\FileSystem.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Archive.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\PE.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\InstallerEvidence.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Installers\Inno.psm1 -Force; Get-InnoInfo -Path `$InstallerPath"
    if ($InnoExtractName) {
      $ExtractName = $InnoExtractName.Replace("'", "''")
      Invoke-InstallerParserBenchmark -Name InnoExtract -Path $InnoPath -Expression "Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Runtime.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Binary.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\FileSystem.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Archive.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\PE.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\InstallerEvidence.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Installers\Inno.psm1 -Force; `$DestinationPath = Join-Path ([IO.Path]::GetTempPath()) ('Dumplings-InnoBenchmark-' + [guid]::NewGuid().ToString('N')); try { Expand-InnoInstaller -Path `$InstallerPath -DestinationPath `$DestinationPath -Name '$ExtractName' -CollisionAction Rename } finally { Remove-Item -LiteralPath `$DestinationPath -Recurse -Force -ErrorAction SilentlyContinue }"
    }
  }
  if ($SetupFactoryPath) {
    Invoke-InstallerParserBenchmark -Name SetupFactory -Path $SetupFactoryPath -Expression "Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Runtime.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Binary.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\FileSystem.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Archive.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\PE.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\InstallerEvidence.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Installers\SetupFactory.psm1 -Force; Get-SetupFactoryInfo -Path `$InstallerPath"
  }
  if ($AdvancedInstallerPath) {
    Invoke-InstallerParserBenchmark -Name AdvancedInstaller -Path $AdvancedInstallerPath -Expression "Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Runtime.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Binary.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\FileSystem.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\Archive.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\PE.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Infrastructure\InstallerEvidence.psm1 -Force; Import-Module .\Modules\InstallerParsers\Libraries\Installers\AdvancedInstaller.psm1 -Force; Get-AdvancedInstallerInfo -Path `$InstallerPath"
  }
