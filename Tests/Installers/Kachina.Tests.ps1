...
BeforeAll {
  ...
  $Script:Akasha = Get-DumplingsTestFixture -RelativePath (Resolve-DumplingsTestFixtureCatalogPath -Name 'AkashaNavigator.Install.1.4.0.exe') `
  ...
  $Script:BetterGiEarly = Resolve-DumplingsTestFixturePath -RelativePath (Resolve-DumplingsTestFixtureCatalogPath -Name 'BetterGI.Install.0.40.0.exe')
  $Script:BetterGiCurrent = Resolve-DumplingsTestFixturePath -RelativePath (Resolve-DumplingsTestFixtureCatalogPath -Name 'BetterGI.Install.0.63.0.exe')
  $Script:MicaSetup = Resolve-DumplingsTestFixturePath -RelativePath (Resolve-DumplingsTestFixtureCatalogPath -Name 'MicaSetup-v2.5.4.exe')
...
Describe 'Kachina analyzer integration' {
  ...
  It 'parses the same installer concurrently in independent runspaces' {
    $ModulePath = (Resolve-Path (Join-Path $Script:DumplingsModuleRoot 'PackageModule.psd1')).Path
