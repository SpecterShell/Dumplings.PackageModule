...
internal partial class Model
{
    ...
    internal Model(bool gentee)
        : this() => _gentee = gentee;

    internal int DecodeGenteeBlock(
        Stream source,
        byte[] buffer,
        int outputSize,
        int modelOrder,
    ...
    )
    {
        if (!_gentee)
        {
            throw new InvalidOperationException("The model is not configured for Gentee PPMd.");
        }
        if (_allocator is null)
        {
            _allocator = new Allocator(gentee: true);
            _allocator.Start(allocatorSize);
        }
        if (modelOrder > 1)
        {
            StartModel(modelOrder, ModelRestorationMethod.Restart);
        }
        else
        {
            if (_maximumContext == PpmContext.ZERO)
            {
                throw new InvalidOperationException("A Gentee continuation block has no initialized model.");
            }
            // Ppmd8Dec uses a fresh symbol mask for every range stream. The
            // managed I1 model generations that mask, so reset its generation
            // before continuing the retained statistical model.
        ...
        var read = DecodeBlock(source, buffer, 0, outputSize);
        if (read != outputSize)
        {
            throw new InvalidDataException(
                $"The Gentee PPMd block ended after {read} of {outputSize} output bytes."
            );
        }
        if (DecodeBlock(source, new byte[1], 0, 1) != 0)
        {
            throw new InvalidDataException("The Gentee PPMd block has data after its declared output.");
        }
        return read;
    }
    ...
    private void StartModel(int modelOrder, ModelRestorationMethod modelRestorationMethod)
    {
        ...
        {
            // Gentee's historical QTable stops the final row at index 196;
            // SharpCompress's standard I1 NS2Indx table extends that row.
            while (
                _probabilities[index1] == probability
                && (!_gentee || probability != 24 || index1 < 196)
            )
            {
                index1++;
            }
    ...
    private void UpdateModel(PpmContext minimumContext)
    {
        ...
        // GEA retains the older PPMd-I behavior: zero OrderFall enters the
        // successor-building path even when the current state has no successor.
        if (_orderFall == 0 && (_gentee || foundStateSuccessor != PpmContext.ZERO))
        {
        ...
        if (foundStateSuccessor != PpmContext.ZERO)
        {
            ...
            // GEA decrements OrderFall only when a real successor was present.
            if (--_orderFall == 0)
            {
        ...
        else
        {
            if (_gentee)
            {
                // The GEA encoder links the state to the just-appended text and
                // continues from MinContext instead of calling ReduceOrder.
                _foundState.Successor = successor;
                foundStateSuccessor = minimumContext;
            }
            else
            {
        ...
        if (!_gentee && _orderFall != 0 && _method > ModelRestorationMethod.Freeze)
        {
            successor = foundStateSuccessor;
